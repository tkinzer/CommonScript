...
(function(global_scope, base_path) {
	...
	/**
	 * Loads dependencies in a synchronous or asynchronous.
	...
	Module.prototype.provide = function(dependencies, callback) {
		...
		if(!async)
			for(var i = 0, l = dependencies.length; i < l; i++)
				this.load(dependencies[i], null);
		else if(dependencies.length)
			this.load(dependencies[0], factoryProvide(this, dependencies, callback));
		else if(callback)
			callback.call(global_scope);
	};

	/**
	 * Loads a module in a synchronous or asynchronous.
	...
	Module.prototype.load = function(id, callback) {
		...
		if(!ownProperty(module_memo, (id = normalizeId(id, this.id))))
			if(!async || !(temp = ownProperty(module_temp, id))) {
				...
				xhr.addEventListener("load", factoryLoad(xhr, id), false);
				xhr.addEventListener("error", factoryError(xhr, id), false);
	...
	/**
	 * Callback factory for method provide.
	 * @param {Module} Object Module.
	 * @param {Array} Module dependencies.
	 * @param {Function} Callback function.
	 * @returns {Function} Callback.
	 */
	function factoryProvide(scope, dependencies, callback) {
		/**
		 * Callback for method provide.
		 */
		function callbackProvide() {
			scope.provide(dependencies.slice(1), callback);
		}

		return callbackProvide;
	}

	/**
	 * Callback factory for XHR event load.
	 * @param {XMLHttpRequest} Object XHR.
	 * @param {String} Normalized module ID.
	 * @returns {Function} Callback.
	 */
	function factoryLoad(scope, id) {
		/**
		 * Callback for XHR event load.
		 * @param {Event} Object XHR event.
		 */
		function eventLoad(event) {
			memoizeModule(id, scope.responseText, false);
		}
	...
	/**
	 * Callback factory for XHR event error.
	 * @param {XMLHttpRequest} Object XHR.
	 * @param {String} Normalized module ID.
	 * @returns {Function} Callback.
	 */
	function factoryError(scope, id) {
		/**
		 * Callback for XHR event error.
		 * @param {Event} Object XHR event.
		 */
		function eventError(event) {
			if(this.status)
				memoizeModule(id, new Error(scope.statusText), true);
			else
				memoizeModule(id, new Error("Fail to load local module."), true);
		}
	...
	function factoryRequire(module) {
		 ...
		 */
		require.memoize = function(id, dependencies, factory) {
	...
	function memoizeModule(id, factory, error) {
		...
		if(ownProperty(module_temp, id)) {
			while(module_temp[id].callback.length)
				module_temp[id].callback.shift().call(global_scope);
	...
	/**
	 * Getter / Setter to configure the base path - Be carefully.
	 */
	...
	/**
	 * Clear all memoized modules.
	 */
	global_scope.require.clear = function() {
		for(var id in module_memo)
			delete module_memo[id].error, module_memo[id].exports, module_memo[id];
	};
