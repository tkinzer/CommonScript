...
(function(global_scope, base_path) {
	...
	Module.prototype.load = function(id, callback) {
		var async = !!callback, xhr;

		if(!ownProperty(module_memo, id = normalizeId(id, this.id)))
			if(!async || !ownProperty(module_temp, id)) {
	...
	function getExports(id) {
		...
		if(!ownProperty(module_memo[id], "exports")) {
			argument[2] = new Module(id);
			argument[1] = module_memo[id].exports = {};
			argument[0] = factoryRequire(argument[2]);
			try {
				if(exports = module_memo[id].factory.apply(null, argument))
	 ...
	 * Public api.
	 */
	global_scope.require = factoryRequire(global_scope.module = new Module(""));
