...
(function(global_scope, base_path) {
	 ...
	 * @param {Array} Module dependencies list.
	 * @param {Function} Callback function.
	 */
	Module.prototype.provide = function(dependencies, callback) {
		var async = !!callback, current;

		if(dependencies.length) {
			current = dependencies.shift();
			if(!async) {
				this.load(current);
				this.provide(dependencies);
			} else {
				this.load(current, factoryProvide(this, dependencies, callback));
			}
		} else if(callback) {
	...
	function factoryProvide(scope, dependencies, callback) {
		...
		function callbackProvide() {
			scope.provide(dependencies, callback);
		}
